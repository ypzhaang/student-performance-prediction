...
if isfield(para, 'max_out');      max_out = para.max_out;       else max_out =0;   end
if isfield(para, 'max_in');       max_in  = para.max_in;        else max_in  = 0;  end  
if isfield(para, 'iter_out');     iter_in = para.iter_in;     else iter_in = 1;  end
...
[m,n]=size(M);
if isfield(para, 'lambda_u');     lambda_u    = para.lambda_u;        else lambda_u   = 99900/(m+n);   end

if isfield(para, 'lambda_v');     lambda_v    = para.lambda_v;        else lambda_v   = 300/(m+n);   end
...
r = size(U,2);
iter_out = 1;
para.iter_out = iter_out;
exceed_max_ratio =0;
while 1
%     disp(iter_out); %% 2018年8月7日17点29分，死循环
    if  iter_out == 1
        DeltaU = zeros(size(U));
        DeltaV = zeros(size(V));
        E = Res;
...
%         disp(['iter_in = ' ,num2str(iter_in)])
        ...
        En = W.*E_temp+ cW.*temp;
%         lambda_u = lambda_u / (sqrt(iter_in + 1));
%         lambda_v = lambda_v / (sqrt(iter_in + 1));
        DeltaUn = (-(lambda_u / sqrt(iter_in))*U+3*beta*V2norm*DeltaU-Yhat*V)./(((lambda_u / sqrt(iter_in))+rdLambda_u+3*beta*V2norm)'*ones(1,r));
